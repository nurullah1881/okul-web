...
if (isset($_POST['ekle'])) {
    ...
    $sql = "INSERT INTO duyurular (duyuru, href) VALUES ('$duyuru', '$href')";
    $conn->query($sql);
...
if (isset($_POST['duzenle'])) {
    ...
    $duyuru = $conn->real_escape_string($_POST['duyurutext']);
    $href = $conn->real_escape_string($_POST['baglanti']);
    $sql = "UPDATE duyurular SET duyuru = '$duyuru', href = '$href' WHERE id = '$id'";
    $conn->query($sql);
    header("Location: " . $_SERVER['PHP_SELF']);
    exit;
}

$duyurular = $conn->query("SELECT * FROM duyurular")->fetch_all(MYSQLI_ASSOC);
$duzenlenecek = isset($_GET['duzenle']) ? $conn->query("SELECT * FROM duyurular WHERE id = '$_GET[duzenle]'")->fetch_assoc() : null;
?>
        ...
        <form action="#" method="POST">
            <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="duyurutext" name="duyurutext" maxlength="50" placeholder="Duyurulacak metni girin..." value="<?= $duzenlenecek ? htmlspecialchars($duzenlenecek['duyuru']) : '' ?>" required>
                </div>
                <div class="mb-3">
                    <label for="baglanti" class="form-label">Bağlantı</label>
                    <input type="text" class="form-control" id="baglanti" name="baglanti" maxlength="255" placeholder="Duyuru bağlantısı..." value="<?= $duzenlenecek ? htmlspecialchars($duzenlenecek['href']) : '' ?>">
                </div>
            </div>
            <div class="d-grid">
                ...
                <?php else: ?>
                    ...
                    <?php foreach ($duyurular as $duyuru): ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars($duyuru['duyuru']) ?></td>
